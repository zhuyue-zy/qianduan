...
import { Button, Table, Icon, Form, Modal, DatePicker, Radio, Select, Tooltip, Popconfirm } from 'yqcloud-ui';
import { injectIntl } from 'react-intl';
import { Content, Header, Page, Permission, Action } from 'yqcloud-front-boot';
...
class AnnouncementHome extends Component {
  ...
  loadAnnouncement = (paginationIn, sortIn, filtersIn, paramsIn) => {
    ...
    AnnouncementStore.loadAnnouncements(this.organizationId, pagination, sort, filters, params)
      .then((data) => {
  ...
  preview = (id) => {
    ...
    AnnouncementStore.getAnnouncementById(this.organizationId, id).then((data) => {
      if (!data.failed) {
        this.setState({ data });
      }
    });
  ...
  /* 创建 */
  createLayout = () => {
    this.props.history.push('/iam/announce/site/create');
  };
   ...
   */
  handleEdit = (record) => {
    this.props.history.push(`/iam/announce/site/edit/${record.announcementId}`);
  };
  ...
  handleDelete = (record) => {
    ...
    const isDeleted = 'Y';
    const status = '已删除';
    AnnouncementStore.deleteAnnouncement(this.organizationId, announcementId, status, isDeleted, objectVersionNumber).then(({ failed, message }) => {
      if (failed) {
   ...
   */
  handleAble = (record) => {
    const { AnnouncementStore, intl } = this.props;
    const { objectVersionNumber, announcementId } = record;
    if (record.isEnabled === 'Y') {
      // 禁用
      const isEnabled = 'N';
      const status = '已失效';
      AnnouncementStore.ableConfiguration(this.organizationId, announcementId, status, isEnabled, objectVersionNumber).then(({ failed, message }) => {
        if (failed) {
          Choerodon.prompt(message);
        } else {
  ...
  render() {
    const { AppState, AnnouncementStore, intl } = this.props;
    const { edit, startValue, endValue, dataSource, data, visible, preview, sort: { columnKey, order }, pagination, filters, params } = this.state;
    ...
    const columns = [{
      ...
      render: (text, record) => {
        if (record.receiver === 1) {
          return (AnnouncementStore.languages.platform);
        } else if (record.receiver === 2) {
          return (AnnouncementStore.languages.Tenant);
        }
      },
    }, {
        ...
        if (record.isEdit) {
          return (
            ...
            // label={AnnouncementStore.languages[ `${intlPrefix}.startTime`]}
              ...
              showTime
              value={startValue}
              defaultValue={moment(record.startTime) || null}
        ...
        if (record.isEdit) {
          return (
            <span>
              <Tooltip placement="top" title={AnnouncementStore.languages.save}>
                <Icon
                  ...
                  onClick={() => {
                    ...
                    AnnouncementStore.updateAnnTime(this.organizationId, {
              ...
              <Tooltip placement="top" title={AnnouncementStore.languages.cancel}>
                <Icon
                  type="yizhongzhi"
                  style={{ color: '#B8BABF' }}
                  onClick={() => {
          ...
          if (record.status === '未发布') {
            actionDatas = [{
              service: ['yqcloud-portal-service.announcement-site.editAnnouncement'],
              type: 'site',
              icon: '',
              text: AnnouncementStore.languages.modify,
              action: this.handleEdit.bind(this, record),
            }, {
              service: ['yqcloud-portal-service.announcement-site.deleteAnUnenableAnnouncement'],
              type: 'site',
              icon: '',
    ...
    return (
      <Page
        service={[
          ...
          'yqcloud-portal-service.announcement-site.addAnnouncement',
          'yqcloud-portal-service.announcement-site.editAnnouncement',
          'yqcloud-portal-service.announcement-site.deleteAnUnenableAnnouncement',
          'yqcloud-portal-service.announcement-site.selectDetail',
          'yqcloud-portal-service.announcement-site.publishAnnouncement',
        ]}
      >
        <Header
        ...
        >
          ...
          <Button
            ...
            onClick={this.reload}
          >
            <Icon type="shuaxin" style={{ color: '#2196F3', width: 25 }} />
            {AnnouncementStore.languages.refresh}
          </Button>
        </Header>
        <Content
