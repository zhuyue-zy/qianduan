...
import { Button, Modal, Tag, Table, Popover, Card, Checkbox, Icon, Form, message, Row, Col, Tooltip } from 'yqcloud-ui';
import CapabilityStore from '../../../../stores/organization/capabilityTag/CapabilityStore';
import './CapabilityStyle.scss';
...
class CapabilityEdit extends Component {
  ...
  loadMemberCapability = (paginationIn, sortIn, filtersIn, paramsIn) => {
    ...
    const { pagination: paginationState, sort: sortState, filters: filtersState, params: paramsState, id } = this.state;
    const { organizationId } = AppState.currentMenuType;
    const pagination = paginationIn || paginationState;
    const sort = sortIn || sortState;
    const filters = filtersIn || filtersState;
    const params = paramsIn || paramsState;
    CapabilityStore.queryMemberCapability(
      organizationId,
      pagination,
      sort,
      filters,
  ...
  capabilityLabelTags=(text) => {
    const labelsArray = [];
    text.forEach((value) => {
      labelsArray.push(<Tag color="cyan" className="label-tooltip-content">{value.name}</Tag>);
    });
    return labelsArray;
  }
 ...
 handleLabels=() => {
   ...
   CapabilityStore.addCapabilityLabels(organizationId, labels).then((data) => {
     if (data === 'success') {
       this.setState({
         capabilityVisible: false,
         selectedRowKeys: [],
       ...
       this.loadMemberCapability();
       Choerodon.prompt(CapabilityStore.languages[`${intlPrefix}.createLabels.success`]);
     }
   });
 }
  ...
  deleteMemberLable=() => {
    ...
    if (optionvalue.length > 0) {
      CapabilityStore.deleteCapabilityLabels(organizationId, getCapability.employeeId, optionvalue).then((data) => {
        if (data === 'success') {
          this.loadMemberCapability();
          Choerodon.prompt(CapabilityStore.languages[`${intlPrefix}.deleteLabels.success`]);
          this.setState({
            checkAll: false,
            editCapabilityVisible: false,
            optionvalue: [],
      ...
      Choerodon.prompt(CapabilityStore.languages[`${intlPrefix}.deleteLabels.tooltip`]);
    }
  }

  onChanges = (checkedValues) => {
    const { optionvalue, getCapability } = this.state;
    this.setState({
  ...
  capabilityTagTable = () => {
    ...
    const columns = [
      {
        title: CapabilityStore.languages[`${intlPrefix}.labelClassify`],
      ...
      {
        title: CapabilityStore.languages[`${intlPrefix}.labelName`],
        dataIndex: 'name',
        key: 'name',
        filters: [],
        width: 150,
  ...
  render() {
    ...
    const { filters, pagination, dataSource, capabilityVisible, selectedCodeValues, selectedRowKeys, editCapabilityVisible, optionvalue, checkboxOptions, getCapability, veiwId, keyArray} = this.state;
    ...
    if (JSON.stringify(getCapability) !== '{}') {
      getCapability.abilityLabels.forEach((v) => {
        // checkboxOption.push({ label: v.name, value: v.id });
        checkboxOption.push(
          <Col span={12}>
            <Checkbox value={v.id}>
              <Ellipsis
    ...
    const columns = [
      { title: CapabilityStore.languages[`${intlPrefix}.menberName`],
      ...
      { title: CapabilityStore.languages[`${intlPrefix}.menberCode`],
      ...
      { title: CapabilityStore.languages[`${intlPrefix}.MobilePhone`],
        dataIndex: 'mobile',
        key: 'mobile',
        filters: [],
      },
      { title: CapabilityStore.languages[`${intlPrefix}.email`],
        dataIndex: 'email',
        key: 'email',
        filters: [],
      },
      { title: CapabilityStore.languages[`${intlPrefix}.entryTime`],
        dataIndex: 'joinDate',
        key: 'joinDate',
        filters: [],
      },
      { title: CapabilityStore.languages[`${intlPrefix}.organization`],
        dataIndex: 'organizationName',
        key: 'organizationName',
        filters: [],
      },
      { title: CapabilityStore.languages[`${intlPrefix}.mainPost`],
        dataIndex: 'positionName',
        key: 'positionName',
        filters: [],
      },
      {
        title: CapabilityStore.languages[`${intlPrefix}.Isproject`],
      ...
      {
        title: CapabilityStore.languages[`${intlPrefix}.labels`],
    ...
    return (
      <Page>
        <Header
          title={CapabilityStore.languages[`${intlPrefix}.memberAblity`]}
          backPath={`/iam/capabilityTag?type=organization&id=${organizationId}&name=${name}&organizationId=${organizationId}&defaultActiveId=${'2'}`}
        >
          <Button
          ...
          >
            ...
            {CapabilityStore.languages[`${intlPrefix}.addLabels`]}
          </Button>
        </Header>
        <Content>
          <Table
            ...
            rowSelection={rowSelection}
            rowKey="id"
            loading={CapabilityStore.isLoading}
            dataSource={dataSource}
            onChange={this.handlePageChange.bind(this)}
            scroll={{ x: 1800 }}
          />
          <Modal
            title={CapabilityStore.languages[`${intlPrefix}.capabilityTags`]}
            ...
            footer={[
              ...
              >
                {CapabilityStore.languages.cancle}
              </Button>,
            ]}
          >
          ...
          <Modal
            title={CapabilityStore.languages[`${intlPrefix}.editCapabilityTags`]}
            ...
            footer={[
              <Button
                ...
                funcType="raised"
              >
                {CapabilityStore.languages.delete}
              </Button>,
              <Button
                onClick={this.closeModalCapa}
                funcType="raised"
          ...
          >
              ...
              extra={(
                <span>
                  ...
                  >
                    ...
                    >
                      {CapabilityStore.languages['select all']}
                    </Checkbox>
                  </div>
            ...
            >
              <div className="checkbox-content">
                {/* <CheckboxGroup options={checkboxOption} value={optionvalue} onChange={this.onChanges} /> */}
                <Checkbox.Group
                  style={{ width: '100%' }}
...
export default Form.create({})(withRouter(injectIntl(CapabilityEdit)));
