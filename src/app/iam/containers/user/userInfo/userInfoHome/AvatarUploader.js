...
import { Button, Form, Icon, Input, Modal, Select, Upload } from 'yqcloud-ui';
...
export default class AvatarUploader extends Component {
  ...
  renderEditor(props) {
    ...
    const maskStyle = {
      ...
      borderRightWidth: editorWidth - x - left - size-40,
    ...
    return (
      <div>
        <h3 className={`${prefixClas}-text`}>
          {UserInfoStore.languages[`${intlPrefix}.text`]}
          <Icon type="keyboard_arrow_right" />
          <span>{file.name}</span>
        </h3>
        <h4 className={`${prefixClas}-hint`}>
          {UserInfoStore.languages[`${intlPrefix}.hint`]}
        </h4>
        <div className={`${prefixClas}-wraper`}>
          ...
          <div className={`${prefixClas}-preview`}>
            <h5 className={`${prefixClas}-preview-title`}>
              {UserInfoStore.languages[`${intlPrefix}.preview`]}
            </h5>
        ...
        <div className={`${prefixClas}-button`}>
          <Upload {...props}>
            <Button icon="file_upload" type="primary">
              {UserInfoStore.languages[`${intlPrefix}.button`]}
            </Button>
          </Upload>
        </div>
      </div>
    ...
    return {
      ...
      beforeUpload: (file) => {
        ...
        if (size > limitSize * 1024) {
          Choerodon.prompt(UserInfoStore.languages[`${intlPrefix}.file.size.limit`]`${limitSize / 1024}M`);
          return false;
        }
        this.setState({ file });
      ...
      (
        <Dragger className="user-info-avatar-dragger" {...props}>
          ...
          <h3 className="user-info-avatar-dragger-text">
            {UserInfoStore.languages[`${intlPrefix}.dragger.text`]}
          </h3>
          <h4 className="user-info-avatar-dragger-hint">
            {UserInfoStore.languages[`${intlPrefix}.dragger.hint`]}{`${limitSize / 1024}M`}
          </h4>
        </Dragger>
      );
  }
  ...
  render() {
    ...
    const modalFooter = [
      ...
      <Button key="save" type="primary" disabled={!img} loading={submitting} onClick={this.handleOk}>
        {UserInfoStore.languages.save}
      </Button>,
    ];
    return (
      <Modal
        title={UserInfoStore.languages[`${intlPrefix}.title`]}
