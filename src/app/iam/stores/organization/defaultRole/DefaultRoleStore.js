...
class DefaultRoleStore extends LanguageStore{
  constructor() {
    super('defaultRole');
  }
...
/*  constructor(totalPage = 1, totalSize = 0) {
  ...
  @action
  handleCanChosePermission( data) {
    this.setCanChosePermission( data.content);
  ...
  getWholePermission( page, filters = {
    code: '',
    params: '',
  }) {
    let filterStr = '';
    ...
    return Observable.fromPromise(axios.get(`iam/v1/permissions/default/list?page=${page.current - 1}&size=${page.pageSize}${filterStr}`));
  }


  loadNewRole = (page, filters) => {
    let filterStr = '';
    for (const i in filters) {
      filterStr += filters[i][0] ? `&${i}=${filters[i][0]}` : '';
    }
    return axios.get(`iam/v1/permissions/default?page=${page.current - 1}&size=${page.pageSize}${filterStr}`);
  };
  ...
  loadRole({ current, pageSize },
    { columnKey, order },
    { name, code, level, enabled, builtIn }, params = [], organizationId=1) {
    const body = {
      name: name && name[0],
      code: code && code[0],
      level: level && level[0],
  ...
  //删除权限
  deleteNewRoles = (role) => {
    return axios.delete(`iam/v1/permissions/default/delete`,{data:JSON.stringify(role)})
  };
  ...
  createRole = (role) => axios.put(`iam/v1/permissions/default/update`, role);
  ...
  /* 查询是否为默认角色 */
  isDefaultRole = () => axios.get('/iam/v1/custom/roles');
}

const roleStore = new DefaultRoleStore();
