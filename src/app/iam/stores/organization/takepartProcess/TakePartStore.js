...
@store('TakePartStore')
class TakePartStore extends LanguageStore{
  ...
  loadTakeParts = (iamOrganizationId, page, sortParam, filters, param) => {
    ...
    return axios.get(
      `workflow/v1/${iamOrganizationId}/participate/participation?page=${page.current - 1}&size=${page.pageSize}&sort=${sortParam}${filter}`,
    ).then((data) => {
  ...
  // 撤回
  callBackProcess=(iamOrganizationId, procInstId, processStatus) => axios.put(`workflow/v1/${iamOrganizationId}/participate/kafka/${procInstId}/participation?processStatus=${processStatus}`)
...
const takepartStore = new TakePartStore();
export default takepartStore;
